...
def convert_a2a_part_to_genai(part: Part) -> types.Part:
    """Convert a single A2A Part (protobuf) into a Google Gen AI Part."""
    if part.text:
        return types.Part(text=part.text)
    if part.url:
        return types.Part(file_data=types.FileData(file_uri=part.url, mime_type=part.media_type))
    if part.raw:
        return types.Part(
            inline_data=types.Blob(data=part.raw, mime_type=part.media_type or "application/octet-stream")
        )
...
def convert_genai_part_to_a2a(part: types.Part) -> Part:
    """Convert a single Google Gen AI Part into an A2A Part (protobuf)."""
    if part.text:
        return Part(text=part.text)
    if part.file_data:
        if not part.file_data.file_uri:
            raise ValueError("File URI is missing")
        return Part(url=part.file_data.file_uri, media_type=part.file_data.mime_type)
    if part.inline_data:
        if not part.inline_data.data:
            raise ValueError("Inline data is missing")
        return Part(raw=part.inline_data.data, media_type=part.inline_data.mime_type)
